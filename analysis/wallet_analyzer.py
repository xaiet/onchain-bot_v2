...
async def _birdeye_price_at(session, mint: str, timestamp: int) -> Optional[float]:
    """Preu USD d'un token en un timestamp concret (resolució 1m)."""
    if not BIRDEYE_API_KEY:
        return None
    url = f"{BIRDEYE_BASE}/defi/history_price"
    params = {
        "address": mint, "address_type": "token", "type": "1m",
...
def _parse_swaps(transactions: list, token_mint: str) -> list:
    ...
    for tx in transactions:
        ...
        if is_buy:
            ...
            swaps.append({
                "type": "BUY", "token_amount": tok_amt, "sol_amount": sol_amt,
                "price_sol": sol_amt / tok_amt if tok_amt else 0,
                "price_usd": None,
                "timestamp": ts, "datetime": dt, "signature": sig,
            })
            ...
            tok_amt = (tok.get("tokenAmount", {}).get("amount", 0) or 0) / (10 ** dec)
            swaps.append({
                "type": "SELL", "token_amount": tok_amt, "sol_amount": sol_amt,
                "price_sol": sol_amt / tok_amt if tok_amt else 0,
                "price_usd": None,
                "timestamp": ts, "datetime": dt, "signature": sig,
            })
...
async def _enrich_usd(session, swaps: list, mint: str) -> list:
    """Afegeix price_usd a cada swap via Birdeye (en paral·lel)."""
...
def _calc_pnl(swaps: list, current_usd: Optional[float] = None) -> dict:
    ...
    # USD: preu USD × quantitat tokens en cada swap
    ...
    return {
        ...
        "time_in_pos": time_str,
        "num_buys": len(buys), "num_sells": len(sells),
        "profitable": realized_sol > 0,
...
async def get_wallet_pnl(wallet: str, token_mint: str) -> str:
    async with aiohttp.ClientSession() as session:
        ...
        if not txs:
            return "❌ No s'han trobat transaccions. Comprova l'adreça o que tingui activitat."
    ...
    if pnl["realized_usd"] is not None:
        usign = "+" if pnl["realized_usd"] >= 0 else ""
        lines.append(f"💵 *PnL USD:* {usign}${pnl['realized_usd']:.2f}")
...
async def get_wallet_score(wallet: str, num_tokens: int = 20) -> str:
    ...
    tokens_to_check = mints_seen[:num_tokens]
    if not tokens_to_check:
        return "❌ No s'han trobat tokens per analitzar."

    results = []
    for mint in tokens_to_check:
        swaps = _parse_swaps(txs, mint)
        if not swaps or not any(s["type"] == "BUY" for s in swaps):
            continue
    ...
    verdicts = [
        (75, "Wallet molt consistent i profitable. Val la pena seguir."),
        (50, "Wallet decent. Bons resultats però no excepcional."),
        (30, "Resultats mixtes. Vigilar abans d'afegir al watchlist."),
        (0,  "Wallet poc profitable. Probablement retail o bot perdedor."),
    ]
...
async def compare_wallets(wallet1: str, wallet2: str, token_mint: str) -> str:
    ...
    lines = [
        f"🔄 *Comparació de Wallets* — ${symbol}",
        "",
    ...
    for label, pnl in [("A", pnl1), ("B", pnl2)]:
        if not pnl:
            lines.append(f"📭 Wallet {label}: sense activitat en aquest token\n")
            continue
        sign  = "+" if pnl["realized_sol"] >= 0 else ""
        emoji = "✅" if pnl["profitable"] else "❌"
        usd_line = ""
        if pnl["realized_usd"] is not None:
            ...
            usd_line = f"\n  • PnL USD: {us}${pnl['realized_usd']:.2f}"
        lines += [
            f"📊 *Wallet {label}:*",
    ...
    lines.append("")
    if signals >= 3:
        lines.append("🚨 *Veredicte: Alta probabilitat de cluster coordinat*")
    elif signals >= 1:
        lines.append("⚠️ *Veredicte: Senyals de coordinació, investigar més*")
    else:
        lines.append("✅ *Veredicte: No semblen coordinades*")
